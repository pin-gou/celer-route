...
> Note: the upstream `docs/openapi/` layout does not exist in this repository — the docs live under `website/` (Docusaurus) and there is no checked-in OpenAPI bundle (generated by `.github/workflows/openapi-bundle.yml` on `main`). This e2e copy is maintained directly under `tests/e2e/api/`, so no upstream sync step is required; the commands below are kept for reference only.

The collection and supporting files are maintained under `tests/e2e/api/`. To refresh from an external OpenAPI source, map:
