 ...
 * custom branding.
 *
 * Enterprise deployments can replace the celer-route logo and icon. Every endpoint
...
export interface BrandingState {
	...
	has_logo: boolean;
	has_icon: boolean;
	/** Content-versioned; empty when the slot uses the celer-route default. */
	logo_url?: string;
	icon_url?: string;
	updated_at?: string;
}
 ...
 *   ""             -> clear that slot, restoring the celer-route default
...
export const brandingApi = baseApi.injectEndpoints({
	endpoints: (builder) => ({
		...
		// Clears branding and restores the default celer-route logo and icon.
