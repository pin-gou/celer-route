							...
							<div className="min-w-0 space-y-0.5">
								...
								<DialogDescription className="text-xs leading-relaxed">
									{exchangeVerifyClient?.state === "pending_verification"
										? "Establish celer-route's discovery credential using your identity."
										: "Renew celer-route's own discovery credential using your identity."}
								</DialogDescription>
							</div>
						</div>
					</DialogHeader>
					<div className="space-y-3 px-5 py-4">
						<InfoBox icon={<KeyRound className="size-4" />}>
							<p>
								...
								{exchangeVerifyClient?.state === "pending_verification" ? "establish" : "renew"} celer-route&apos;s discovery credential for{" "}
										...
										tooltip={
											<>
												<p>
													...
													celer-route's own connection checks to the server, not the results of tool calls made through it: it self-heals
													and never blocks tool calls. For per-user clients (OAuth, headers, token exchange), this reflects celer-route's
													own retained admin credential (used only to periodically refresh the tool list), not any individual user's own
										...
										<TableRow key={c.config.client_id} className="group hover:bg-muted/50 transition-colors">
											...
											<TableCell onClick={(e) => e.stopPropagation()}>
												{/* Every state (healthy/unstable/needs_reauth/pending_verification)
												    ...
												    that "unstable" reflects celer-route's own connection checks, not
