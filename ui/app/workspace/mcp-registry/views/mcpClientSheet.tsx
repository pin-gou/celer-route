							...
							<div className="space-y-2">
								...
								<SheetDescription>
									{mcpClient.state === "pending_verification"
										? mcpClient.config.auth_type === "token_exchange"
											? "This server needs a one-time verification: celer-route exchanges your signed-in identity token, tests the connection, and discovers tools. Callers then have their own identity tokens exchanged automatically on every tool call."
											: mcpClient.config.auth_type === "per_user_oauth"
												? "This client was declared in config.json. An admin sign-in is needed to verify the OAuth setup and discover tools; celer-route keeps it on file to refresh the tool list periodically. Each user will still authenticate individually when they use this server."
										...
										: mcpClient.state === "needs_reauth"
											? isPerUserAuth
												? "The admin credential celer-route keeps on file to refresh this server's tool list needs repair. End-user credentials and tool calls are unaffected. Use Refresh admin credential from the server's actions menu to fix it."
