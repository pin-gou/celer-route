...
function TopDimensionTooltip({ active, payload }: any) {
	...
	return (
		<div className="rounded-sm border border-zinc-200 bg-white px-3 py-2 shadow-lg dark:border-zinc-700 dark:bg-zinc-900">
			...
			<div className="text-sm font-medium">{data.total_requests.toLocaleString()} {t("charts.requests")}</div>
		</div>
	);
}
...
function TopDimensionChart({
	...
	return (
		<ChartCard
			title={t("rankings.topDimension", { dimensionLabel })}
			loading={loading}
			testId={`${testIdPrefix}-top-chart`}
			className="z-[1] h-full"
			totalLabel={attributed && actualTotal === null ? t("charts.totalRequestsAttributed") : t("charts.requests")}
			total={
				actualTotal !== null ? (
			...
			totalTooltip={
				grandTotal === null ? undefined : actualTotal !== null ? (
					<div className="max-w-[240px] text-xs opacity-80">{t("charts.actualRequests")}</div>
				) : attributed ? (
					<div className="space-y-1">
						<div className="max-w-[240px] text-xs opacity-80">
							{t("charts.attributedDescription", { dimensionLabel: dimensionLabel.toLowerCase() })}
						</div>
					</div>
				) : (
			...
			secondaryTotalLabel={t("charts.attributedRequests")}
		...
		>
			<div style={{ height: Math.max(200, chartData.length * 40 + 40), marginBottom: 6 }}>
				{chartData.length > 0 ? (
					...
					<div className="text-muted-foreground flex h-full items-center justify-center text-sm">{t("empty.noData")}</div>
				)}
			</div>
			<div className="py-2">
			...
			{loading ? (
				...
				<Card className="rounded-sm p-4 shadow-none">
					<div className="text-muted-foreground flex h-[200px] items-center justify-center text-sm">
						{t("empty.noDimensionUsage", { dimension: dimensionLabel.toLowerCase() })}
					</div>
				</Card>
			) : (
				<Card className="rounded-sm p-2 shadow-none" data-testid={`${testIdPrefix}-table`}>
					<span className="text-primary pl-2 text-sm font-medium">{t("rankings.dimensionRankings", { dimension: dimensionLabel })}</span>
					<Table>
						<TableHeader>
							<TableRow>
