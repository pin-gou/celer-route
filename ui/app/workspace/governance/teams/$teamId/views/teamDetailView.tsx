		...
		<div className="flex h-full flex-col gap-4 p-6" data-testid={`team-detail-${teamId}`}>
			<header className="flex flex-wrap items-end justify-between gap-3">
				...
				<div className="flex items-center gap-2">
					...
					<Button onClick={() => setInviteSheetOpen(true)} data-testid="team-detail-invite">
			...
			{pendingRevoke && (
				<Card data-testid="team-detail-revoke-confirm">
					<CardHeader>
						<CardTitle className="text-base">{t("revokeConfirmTitle")}</CardTitle>
						<CardDescription>{t("revokeConfirmDesc")}</CardDescription>
					</CardHeader>
					<CardContent className="flex gap-2">
						<Button onClick={() => onRevoke(pendingRevoke)} data-testid="team-detail-revoke-confirm-action">
							{t("confirmRevoke")}
						</Button>
			...
			{inviteLink && (
				<Card data-testid="team-detail-invite-link">
					<CardHeader>
					...
					<CardContent>
						<div className="bg-muted flex items-center gap-2 rounded-sm p-3 font-mono text-xs">
							<code className="flex-1 break-all" data-testid="team-detail-invite-token">
								{inviteLink.token}
							</code>
							<Button variant="ghost" size="icon" onClick={() => copy(inviteLink.token)} data-testid="team-detail-invite-token-copy">
			...
			<button hidden onClick={() => refetchMembers()} data-testid="team-detail-refetch-members" />
		</div>
	);
}
...
function InviteSheet({
	...
	return (
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="w-full sm:max-w-md" data-testid="team-detail-invite-sheet">
				<SheetHeader>
				...
				<form onSubmit={handleSubmit} className="space-y-4 px-4 pb-4">
					<div className="space-y-2">
						<Label htmlFor="invite-email">{t("col_email")}</Label>
						<Input
							id="invite-email"
							type="email"
							value={email}
					...
					<div className="space-y-2">
						...
						<Select value={role} onValueChange={(v) => setRole(v as "admin" | "member")}>
							<SelectTrigger data-testid="team-detail-invite-role">
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
					...
					<SheetFooter className="px-0">
						...
						<Button type="submit" isLoading={isSubmitting} data-testid="team-detail-invite-submit">
							{t("invite")}
						</Button>
					</SheetFooter>
				</form>
