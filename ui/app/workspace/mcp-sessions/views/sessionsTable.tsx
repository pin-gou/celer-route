					...
					<AlertDialogHeader>
						{pendingDelete?.kind === "header" ? (
							<>
								...
								<AlertDialogDescription>
									celer-route will remove the stored credential values for this binding. There is no upstream token to revoke; the user will
							...
							<>
								...
								<AlertDialogDescription>
									celer-route will remove the stored credential for this binding. The upstream OAuth token is not revoked at the provider;
									it stays detached and expires naturally. Anyone using this binding will need to re-authenticate to obtain a fresh token.
								</AlertDialogDescription>
							</>
						)}
					</AlertDialogHeader>
							...
							<TableRow>
								...
								<TableHead>
									<HeaderWithTooltip
										...
										tooltip="When the current access token expires. celer-route auto-refreshes using the refresh token on the next request, so an active row past its expiry will silently mint a new token at use time. Header rows do not have an upstream expiry; their values stay valid until revoked or the schema changes."
									/>
								</TableHead>
