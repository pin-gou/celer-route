...
describe("NoPermissionView", () => {
  it("should render the entity string when only entity prop is provided (backward compatibility)", () => {
    ...
    expect(
      screen.getByText(/mcp registry/i),
      "Should display the entity string when entityI18nKey is not provided"
    ).toBeTruthy();
  });
  ...
  it("should display translated text when entityI18nKey is provided", () => {
    ...
    render(
      <NoPermissionView
        entity="mcp registry"
        entityI18nKey="mcp:registry.permissionDenied"
      />
    );

    expect(
      screen.getByText(/MCP 注册中心/i),
      "Should display the translated text from entityI18nKey"
    ).toBeTruthy();
  });
  ...
  it("should fall back to entity string when entityI18nKey is provided but translation returns the key itself", () => {
    ...
    render(
      <NoPermissionView
        entity="unknown module"
        entityI18nKey="nonexistent:route.key"
      />
    );
