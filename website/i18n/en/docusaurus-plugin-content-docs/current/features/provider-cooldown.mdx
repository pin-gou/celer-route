...
| **Effect during cooldown** | The failing `(provider, key)` is skipped in key selection for the cooldown duration (default 300 s); requests fall through to other healthy keys under the same provider, then in order to other providers. If every available key is cooling, requests get a synthetic error `503 no_eligible_keys`. |
