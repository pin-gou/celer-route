...
「生成的配置」区块按顺序给出标签页：**一键应用**（一条命令写入本机 → 见下文）、各
配置文件的完整内容、「环境变量」配方，以及最右侧「用 curl 测试」。下方是各客户端的实际产物
（Base URL = `http://localhost:8080`，API Key 已创建）。
...
「生成的配置」区块最前面有一个「**一键应用**」标签页：里面是一条自包含命令（**bash** 用于
macOS/Linux，**PowerShell** 用于 Windows），复制后在本地终端粘贴运行，即可把 celer-route 的配置
并进对应路径——不需要下载任何二进制，也不依赖网关在线。

命令**不会覆盖你已有的配置**，而是先备份、再精准合并：

1. **备份**：如果目标配置文件已存在，先复制一份 `*.bak-<时间戳>` 放在同目录
2. **精准插入**（按客户端类型）：
   - **JSON**（opencode、Claude Code、WorkBuddy、CodeBuddy）：深度合并——把 `celer-route` 的
     provider / `env` / `models[]` 写进去，**已有字段、其他 provider、其他设置全部保留**
   - **TOML**（Codex）：只替换 `model` / `model_provider` 与 `[model_providers.celer-route]` 段，
     其他段落不动
   - **`.env`**（通用 OpenAI 兼容、MarsCode）：按 KEY 设置或追加，其他行保留
3. **安全失败**：如果已有配置损坏（不是合法 JSON/TOML），命令会**中止并保持原文件不动**（备份保留），
   绝不拿新配置顶掉坏文件

macOS/Linux 的合并用 `python3` 完成（开发机基本都自带）；没有 `python3` 时命令会明确报错而不是
覆盖。Windows 用 PowerShell 内置的 JSON/cmd 函数，零额外依赖。如果你想对配置做临时调整，改
「生成配置」标签页里的内容再重新复制即可。

### macOS / Linux（bash）

以 opencode 为例（先备份，再合并 `provider.celer-route` 与 `model`，其余全保留）：

```bash
mkdir -p "$HOME/.config/opencode"
if [ -f "$HOME/.config/opencode/opencode.json" ]; then
  cp -p "$HOME/.config/opencode/opencode.json" "$HOME/.config/opencode/opencode.json.bak-$(date +%Y%m%d-%H%M%S)" || exit 1
fi
command -v python3 >/dev/null 2>&1 || { echo "python3 is required" >&2; exit 1; }
python3 - <<'PYEOF'
import os, sys, json, base64
p = os.path.expanduser("~/.config/opencode/opencode.json")
ours = json.loads(base64.b64decode("<页面生成的 base64>").decode("utf-8"))
root = json.load(open(p, encoding="utf-8")) if os.path.exists(p) else {}
def deep(base, over):
    for k, v in over.items():
        if isinstance(v, dict) and isinstance(base.get(k), dict):
            ...
            base[k] = v
deep(root, ours)
json.dump(root, open(p + ".tmp", "w", encoding="utf-8"), indent=2, ensure_ascii=False)
os.replace(p + ".tmp", p)
PYEOF
```

> 页面里是完整的可粘贴命令（含备份与解析失败保护），上面只是结构示意。

### Windows（PowerShell）

以 Codex 为例（`$env:USERPROFILE\.codex\config.toml`），先备份，再只替换
`model` / `model_provider` 与 `[model_providers.celer-route]` 段：

```powershell
$__p = "$env:USERPROFILE\.codex\config.toml"
if (Test-Path -LiteralPath $__p) {
  Copy-Item -LiteralPath $__p -Destination "$__p.bak-$((Get-Date).ToString('yyyyMMdd-HHmmss'))" -Force
}
$__oursText = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String("<页面生成的 base64>"))
$__mark = '[model_providers.celer-route]'
# ……合并逻辑见页面完整命令……
[IO.File]::WriteAllText($__p, $__composedText, (New-Object System.Text.UTF8Encoding($false)))
```

### 覆盖范围

- **写文件类**（opencode、Claude Code、Codex、WorkBuddy、CodeBuddy）：命令先备份再精准合并各自的配置文件
- **环境变量类**（通用 OpenAI 兼容、MarsCode）：命令按 KEY 合并当前目录的 `.env`（其他行保留），并提示
  `source .env`（Windows 下提示改用「环境变量」标签页里的 PowerShell / cmd 写法）
- **应用内步骤类**（Cursor、Trae、ZCode、通义灵码）：这些客户端只能在应用内配置，不提供「一键应用」标签页

### 手动/命令行场景

想在不打开页面的场景（CI、远程机器）查看模型目录，仍可使用轻量命令：`celer-route-setup models`
（源码在 `cli/`，无模板逻辑，不需要为各系统/架构分发多份配置生成二进制）。