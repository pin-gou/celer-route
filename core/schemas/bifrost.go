...
// PerKeyFailureMarker is an optional hook called when the retry loop in
// executeRequestWithRetries observes a per-key failure that will cause the
// credential to be excluded from this request (usedKeyIDs/deadKeyIDs).
//
// Unlike PostLLMHook — which only fires once with the terminal outcome and
// therefore never sees the 429 a later retry succeeded around — this hook
// fires *per failed attempt*, letting cross-request key-state (e.g. the
// provider-cooldown plugin) reflect "this key was rate-limited just now"
// even when the request as a whole ultimately succeeds.
//
// provider is the provider the failed attempt targeted.
// err is the BifrostError from the failed attempt; non-nil.
// model is the model string the request used.
//
// The hook is fire-and-forget: errors are logged, never returned. A nil
// pointer or hook returning silently is the no-op path when no plugin
// wants to observe per-key failures.
type PerKeyFailureMarker func(ctx *BifrostContext, provider ModelProvider, keyID string, keyName string, model string, err *BifrostError)
...
const (
	...
	BifrostContextKeyGatewayBaseURL                      BifrostContextKey = "bifrost-gateway-base-url"                         // string — public base URL of the celer-route gateway itself, resolved at request time (config override → Host header fallback). Used by plugins to build self-referential URLs embedded in tool output recovery hints.
	...
	BifrostContextKeyCooldownPolicy                      BifrostContextKey = "bifrost-cooldown-policy"                    // *CooldownPolicy (set by bifrost before PreProviderHook - the per-provider policy the provider-cooldown plugin should consult; nil means use DefaultCooldownPolicy)
	...
	BifrostContextKeyMemberUserID                        BifrostContextKey = "bifrost-member-user-id"        // string (set by MemberAuthMiddleware when a member session is valid - the TableUser.ID for the authenticated member; empty when the request is not on a member-authenticated path)
	...
	BifrostContextKeyRTKBypassedTruncated                BifrostContextKey = "x-bf-rtk-bypassed-truncated"    // []int (set by compression plugin - message/block indices whose tool output was recognised as already-truncated RTK output (it carried a raw-output sentinel) and passed through unchanged via the anti-recursion bypass; lets the log detail diff view surface "echoed truncated content, no new compression" instead of "compression not triggered")
...
// RTKRecoveryHintMarker is the distinctive leading marker of the RTK recovery
// hint message that the RTK plugin prepends at input[0] of every LLM request
// once RTK is enabled (the full hint body lives in plugins/rtk/hint.go
// rtkRecoveryHintText). It is shared so the logging plugin can record — and the
// log detail UI can consume — whether the persisted input array carries the
// gateway-injected hint at its head, without duplicating the byte-stable hint
// body. U+2011 NON-BREAKING HYPHEN ("‑") matches the hint literal exactly.
const RTKRecoveryHintMarker = "[rtk\u2011recovery]"
