...
// assertSingleStreamError checks the stream carried exactly one error for one
// dead stream, via the truncation guard (not the read-error-fast-path that the
// removed SSEReaderFactory injection seam previously exercised).
func assertSingleStreamError(t *testing.T, chunks []*schemas.BifrostStreamChunk) {
	t.Helper()
...
func TestImageGenerationStreamReadErrorReportsOnce(t *testing.T) {
	// A live upstream that dies mid-SSE (previously reproduced via the
	// SSEReaderFactory injection seam) surfaces as exactly one error on the
	// stream — never a synthetic clean completion, never a duplicate.
	server := truncatingSSEServer(t, "data: "+openAIPartialImageEvent+"\n")
	defer server.Close()
	...
	stream, bifrostErr := provider.ImageGenerationStream(
		newStreamTestContext(), passthroughPostHook, nil, testKey(), request)
	if bifrostErr != nil {
		t.Fatalf("stream setup failed: %v", bifrostErr)
	}
	assertSingleStreamError(t, collectChunks(t, stream))
}

func TestImageEditStreamReadErrorReportsOnce(t *testing.T) {
	server := truncatingSSEServer(t, "data: "+openAIPartialImageEditEvent+"\n")
	defer server.Close()
