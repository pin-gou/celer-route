...
- ❌ **禁止**修改 `modules[*].root` 之外的目录（项目根目录的 `.pg/`、开发工具适配目录、`README.md` 等都不允许动）
...
```bash
ENV_RESOLVED=$(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-env <env_name>)
...
2. **用 helper 拿命令**（不要直接读 `modules[<target_module>].test.unit` 字段，因为可能是 dict 形式）：
   ```bash
   RESOLVED=$(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-module-test <target_module> unit)
