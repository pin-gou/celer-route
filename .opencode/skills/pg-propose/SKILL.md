...
| 附录 A：TodoWrite 13 项清单 | [附录 A](#附录-atodowrite-13-项清单) |
...
**TodoWrite 13 项清单详见 [附录 A](#附录-atodowrite-13-项清单)**。本步骤仅在对话开始时执行一次，创建结构化待办，后续每完成一个阶段动作立刻更新对应项状态。
...
> **与正文阶段的映射关系**：
> - 阶段 1.1（TodoWrite 初始化）本身是创建本清单的动作，不计入 13 项
...
> - 阶段 2.4 / 2.6 拆分后，每个子步骤共享一个 TodoWrite 项（2.4 → 9, 2.6 → 11），完成时同时更新
...
- **v1.2.0（2026-08-05）**：pg-1-define「定界后环境验证」产物接入（define-summary.yaml）。
  ...
  - **TodoWrite 12 项 → 13 项**：插入第 5 项（阶段 1.8），后续项编号顺延 +1。
