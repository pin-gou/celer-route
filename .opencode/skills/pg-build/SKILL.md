...
```bash
RUNNER="python3 .pg/skills/src/core/workflows/skills/pg-build/scripts/pg-pipeline-runner.py"
...
# 组合使用
python3 .pg/skills/src/core/workflows/skills/pg-build/scripts/pg-list-phases.py <change> --with-progress --detect-sub-pipelines
```
...
python3 .pg/skills/src/core/workflows/skills/pg-build/scripts/pg-pipeline-runner.py \
...
2. `agent` 字段指定 sub-agent 类型（如 `pg-build/review`），**必须** 作为 Task tool 的 `subagent_type` 参数严格使用，禁止替换为其他 agent。
...
```bash
# 1. 归档当前 change
python3 .pg/skills/src/core/workflows/skills/pg-archive/scripts/pg-archive.py move <change> \
...
- **v2.6 Code View**:
  ...
  - `.pg/skills/src/core/workflows/agents/pg-build/review.md` (sub-agent 定义)
  - `.pg/skills/src/core/workflows/agents/pg-build/fix-review.md` (fix sub-agent 定义)
- **v2.4 result.json 落盘**:
...
- **v3.13 TODO 列表协议**:
  ...
  - `.pg/skills/src/core/workflows/agents/pg-manager.md` (TODO 协议段)
  - `.pg/skills/src/core/workflows/commands/pg-3-build.md` (步骤 2.5 + 3 内嵌调 pg-list-phases.py)
...
- verify agent 协议：`.pg/skills/src/core/workflows/agents/pg-build/verify.md` — §2 / §4.1
...
- agent 协议：`.pg/skills/src/core/workflows/agents/pg-build/scenario-fix.md` — §Step 3.5
