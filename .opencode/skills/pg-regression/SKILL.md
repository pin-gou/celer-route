...
│       ├── 2-agent.log          # opencode run --agent 的 stdout + stderr
...
```bash
python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py pg-regression --suite {suite}
```
...
{"command": $(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-module-test <module> <test_key>), "suite": "<suite>", "skill": "pg-regression", "env": "<env>", "module": "<module>", "log_path": "${RUN_DIR}/temp/<suite>-<test_key>-test-output.log"}
EOF
```
...
```bash
python3 .pg/skills/src/runtime/lib/pg-run-hook.py <<EOF
{"command": $(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-module-test frontend e2e), "suite": "frontend", "skill": "pg-regression", "env": "dev-local", "module": "frontend", "log_path": "${RUN_DIR}/temp/frontend-e2e-test-output.log"}
EOF
```
...
```bash
python3 .pg/skills/src/runtime/lib/pg-run-hook.py <<EOF
{"command": $(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-module-test backend unit), "suite": "backend", "skill": "pg-regression", "env": "dev-local", "module": "backend", "log_path": "${RUN_DIR}/temp/backend-unit-test-output.log"}
EOF
```
...
```bash
python3 .pg/skills/src/runtime/lib/pg-run-hook.py <<EOF
{"command": $(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-module-test agent unit), "suite": "agent", "skill": "pg-regression", "env": "dev-3tier", "module": "agent", "log_path": "${RUN_DIR}/temp/agent-unit-test-output.log"}
EOF
python3 .pg/skills/src/runtime/lib/pg-run-hook.py <<EOF
{"command": $(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-module-test agent integration), "suite": "agent", "skill": "pg-regression", "env": "dev-3tier", "module": "agent", "log_path": "${RUN_DIR}/temp/agent-integration-test-output.log"}
EOF
```
...
```bash
RESOLVED=$(python3 .pg/skills/src/core/workflows/scripts/pg-parse-config.py --resolve-module-test <module> <test_key>)
...
bash .pg/skills/src/core/workflows/skills/pg-regression/scripts/pg-record-fix-test.sh \
  --run-dir "$RUN_DIR" \
  --idx "$IDX" \
  --target "$TARGET" \
...
python3 .pg/skills/src/core/workflows/skills/pg-regression/scripts/pg-regression-summary.py \
...
" .pg/regression/*.json 2>/dev/null; then
  nohup python3 .pg/skills/src/core/workflows/skills/pg-regression/scripts/pg-fix-regression-runner.py \
    --run-dir "$RUN_DIR" \
...
| 问题 | 原因 | 解决 |
|------|------|------|
| fix-test 未确认就绪 | agent 指令加载失败 | 检查 `.pg/skills/src/core/workflows/agents/pg-regression/fix-test.md` |
