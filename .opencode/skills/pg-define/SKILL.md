...
>
> 方法：调用 `Task tool`，将 `subagent_type` 设为 `explore`。在 prompt 中提示 explore agent **优先使用 codegraph 系列工具**。
...
- 当设计树暴露出多个方案分支时，用 `question` tool 一次性呈现方案对比，把方案作为选项，每个选项的 `description` 写清楚核心权衡。推荐方案放在第一个选项并标注"(推荐)"
