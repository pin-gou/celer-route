...
func (h *ConfigHandler) getConfig(ctx *fasthttp.RequestCtx) {
	...
	if h.store.ConfigStore != nil {
		...
		// UI can fall back to the celer-route-admin CLI when the operator left it
	...
	if payload.AuthConfig != nil {
		...
		if payload.AuthConfig.IsEnabled {
			...
			if authConfig == nil && !h.configManager.ValidateSetupToken(payload.AuthConfig.SetupToken) {
				SendError(ctx, fasthttp.StatusForbidden, "a valid setup token is required to create the initial admin account; configure setup_token in config.json (or the BIFROST_SETUP_TOKEN env var) and pass it in this request, or create the admin account with the celer-route-admin admin reset CLI")
				return
			}
