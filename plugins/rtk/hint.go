...
const rtkRecoveryHintText = "[rtk‑recovery] celer‑route (LLM gateway) RTK compression can truncate tool_result blocks. Only when tool_result ends with `[rtk:raw_output_id=<24hex>; orig=<size>; ttl=24h; redacted=true[; fetch=GET <url>]]`, original output saved on celer-route (default 24h TTL, secrets redacted).\n- `orig=`：original output size, to evaluate if recovery is worthwhile.\n- `fetch=`：if present, copy‑pasteable GET URL (no Authorization header), returns redacted raw output as text/plain. Use commands like `curl -s http://localhost:9080/api/context/rtk/raw-output/d32ab592936056c95698bfa0` (example) directly without saving it to disk, only this way, the result will not be truncated again\n- No `fetch=`：recovery unavailable; celer-route lacks resolvable base URL, raw path cannot be accessed."
