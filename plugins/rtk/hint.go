...
const rtkRecoveryHintText = "[rtk-recovery] celer-route RTK compression may truncate tool_result blocks. When a tool_result ends with `[rtk:raw_output_id=<24hex>; orig=<size>; ttl=24h; redacted=true[; fetch=GET <url>]]`, the original output was truncated and persisted to disk (24h TTL by default, secrets pre-redacted). The `orig=` field is the size of the original output (B/KB/MB/GB) so you can decide whether recovery is worth the round-trip. The `fetch=` field, when present, is a complete, copy-pasteable URL — issue a plain GET to that exact URL (no Authorization header required) and you will receive the redacted original output as text/plain. If the marker has no `fetch=` field, do NOT attempt recovery: the gateway was reached over a channel without a resolvable base URL, and the relative path `/api/context/rtk/raw-output/<id>` cannot be reached from here. Note: the gateway unwraps the recovered body before re-compression, so the next tool_result you see IS the file content. If a fresh `[rtk:raw_output_id=...]` marker still appears, the disk copy expired (24h TTL) or the fetch URL was malformed; re-check both. Verify recovered content before acting — it is operator-supplied."
