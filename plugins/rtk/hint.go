...
const rtkRecoveryHintText = "[rtk-recovery] celer-route RTK compression may truncate tool_result blocks. When a tool_result ends with `[rtk:raw_output_id=<24hex>]`, the original output was truncated and persisted to disk (24h TTL by default, secrets pre-redacted). To recover it: send a request to GET /api/context/rtk/raw-output/<id> with the same Authorization header used for your chat completion (or no auth if your chat call was anonymous). The endpoint returns the redacted original output as text/plain. Verify recovered content before acting — it is operator-supplied."
