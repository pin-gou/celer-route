...
func (p *Plugin) PreLLMHook(ctx *schemas.BifrostContext, req *schemas.BifrostRequest) (*schemas.BifrostRequest, *schemas.LLMPluginShortCircuit, error) {
	...
	//
	// Injected AFTER the compression pipeline: the pipeline records
	// ScannedIndices / RawOutputEntries against the array as it received it,
	// and the hint must not shift those positions. When the hint is already
	// present at input[0] (a fallback inherits the primary's injected array),
	// the pipeline's hint-scan offset keeps the recorded indices canonical —
	// see chatHintScanOffset / responsesHintScanOffset.
	if p.config.Enabled {
		injectRtkRecoveryHint(ctx, req)
	}
