...
// isToolResultBlock checks if a ChatContentBlock is an Anthropic-style tool_result block.
// This function will be implemented in anthropic.go.
...
// buildToolCallLookup extracts tool_call_id → tool metadata from assistant messages.
// This function will be implemented in anthropic.go.
func buildToolCallLookup(messages []schemas.ChatMessage) map[string]ToolCallInfo {
		...
		for _, tc := range msg.ChatAssistantMessage.ToolCalls {
			if tc.ID != nil {
				lookup[*tc.ID] = ToolCallInfo{
					ToolName: ptrToStr(tc.Function.Name),
...
// ToolCallInfo holds metadata about a tool call for compression decisions.
// This struct will be defined in anthropic.go (or state.go).
