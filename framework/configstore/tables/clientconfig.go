...
type TableClientConfig struct {
	...
	LogLevel                              string                         `gorm:"default:''" json:"log_level,omitempty"`                    // Application console log level (debug/info/warn/error); empty = follow boot args (LOG_LEVEL / -log-level)
	LogOutputStyle                        string                         `gorm:"default:''" json:"log_output_style,omitempty"`             // Application console log output format (json/pretty); empty = follow boot args (-log-style)
	...
	CelerRouteBaseURL                     string                         `gorm:"type:varchar(512)" json:"celer_route_base_url,omitempty"`         // Public base URL of the celer-route gateway, used to build self-referential URLs embedded in tool output recovery hints (e.g. the RTK raw-output fetch URL). Empty falls back to dynamic Host-header-derived URL.
