# celer-route + PostgreSQL 一体化部署示例

面向生产级 / 多节点场景：celer-route 与 PostgreSQL 16+ 通过 docker-compose 一同编排。
celer-route 使用 PostgreSQL 作为 `config_store` 和 `logs_store`，获得跨节点一致性、
连接池调优与 celer-route 专属的物化视图 / GIN 索引优化。

## 前置条件

- Docker Engine 20.10+
- Docker Compose v2（`docker compose` 子命令）
- 至少 2 GB 可用内存（PostgreSQL + celer-route）
- PostgreSQL **16 或更高版本**（celer-route logstore 硬性要求 ≥ 16，低于此版本将拒绝启动）

## 目录结构

```
.
├── docker-compose.yml   # celer-route + postgres:16-alpine 编排
├── data/
│   └── config.json      # celer-route 配置（postgres 模式）
...
# 3. 查看启动状态（注意等 Postgres healthy 后 celer-route 才会启动）
docker compose ps
```
...
```
NAME            SERVICE       STATUS
celer-route-pg   postgres      Up (healthy)
celer-route      celer-route    Up (healthy)
```
...
两个容器位于默认 bridge 网络 `celer-route-postgres_default` 中。
celer-route 通过 **服务名** `postgres`（而非 `localhost`）访问数据库 —— Docker 在同一网络内
...
| celer-route | `/app/data` | `./data:/app/data` | 仅持久化 `config.json` |
...
docker compose exec -T postgres \
  ...
  | gzip > celer-route-$(date +%Y%m%d).sql.gz
```

**恢复**：

```bash
gunzip -c celer-route-YYYYMMDD.sql.gz | \
  docker compose exec -T postgres \
...
数据库 schema 变更由 celer-route 进程启动时自动迁移，无需手工执行。
跨节点升级时，多副本之间通过 PostgreSQL advisory lock 串行化迁移，保证一致性。

## 从 SQLite 版迁移过来

celer-route **目前未提供自动的 SQLite → PostgreSQL 数据迁移工具**。

如需把现有 SQLite 数据搬到 PostgreSQL，请按以下顺序操作：

1. **导出旧数据**：通过 celer-route 管理界面 / API 把所有 Virtual Key、Provider Key、
   Model Config、Routing Rule 等配置记录下来（建议截图或导出 CSV）。
2. **启动新环境**：`docker compose up -d` 后 celer-route 会在空 PostgreSQL 库中自动建表。
...
**⚠️**：`config.json` 中的 `${POSTGRES_PASSWORD}` 占位符 **不会被 celer-route 解析**。
celer-route 启动时按字面量读取此字符串。本仓库示例中使用明文占位仅为了演示 schema
...
- **方式 A（推荐）**：在挂载前用 `envsubst` 渲染模板：
  ...
  docker compose up -d
  ```
- **方式 B**：使用 `password_command` 字段，让 celer-route 启动时执行命令获取动态凭证
...
# 启动 celer-route
docker compose up -d celer-route
```

### 横向扩容（多 celer-route 实例）

PostgreSQL 后端天然支持多副本。复制 `celer-route` 服务定义、改 `container_name`
即可共享同一 Postgres：

```yaml
services:
  celer-route-1:
    image: ghcr.io/pin-gou/celer-route:latest
    ports: ["8080:8080"]
    ...
