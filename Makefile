...
setup-workspace: ## Set up Go workspace with all local modules for development
	...
	@go list -m all | grep "github.com/pin-gou/celer-route" | grep -v " v" | sed 's/^/  ✓ /'
	@$(ECHO) ""
	@$(ECHO) "$(CYAN)Remote modules (no local version):$(NC)"
	@go list -m all | grep "github.com/pin-gou/celer-route" | grep " v" | sed 's/^/  → /'
	@$(ECHO) ""
...
HARNESS_PROVIDERS := openai anthropic bedrock gemini vertex azure passthrough openrouter gmicloud
		...
		for p in $$PROVIDERS; do \
			...
			( \
				newman run "tmp/harness-filtered-$$p.json" \
					...
					$${GMI_API_KEY:+--env-var "gmicloudKey=$$GMI_API_KEY"} \
