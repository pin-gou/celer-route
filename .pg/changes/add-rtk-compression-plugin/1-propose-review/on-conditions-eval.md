...
|---|---|---|---|
| scr | **true** | explicit | 跨 role 协作验证? 否（仅 pg-gateway 单 service）; 新 API 端点? 否（plugin 内部行为）；跨模块联调? 是（core schema + transports server register + plugins 新模块三处联动），通过 scr scenario 联调验证 verifiable V-* |
