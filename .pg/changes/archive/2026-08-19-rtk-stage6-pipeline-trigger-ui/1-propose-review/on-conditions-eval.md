...
|---|---|---|---|
| scr | **true** | explicit | 跨多个 role 协作验证=是（plugins 接口+transports schema+ui 表单三模块联动）；新 API 端点=否（复用现有 PATCH /api/plugins/rtk）；跨模块联调=是（V-scr-1/2 验证配置→持久化→生效全链路）。因此 scenario-scr 启用。 |
