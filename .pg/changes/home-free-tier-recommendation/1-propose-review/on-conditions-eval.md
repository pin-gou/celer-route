...
|---|---|---|---|
| scr | **true** | explicit | 跨 ui-dev + celer-route-api + logs-db 三 role 联调；新增 2 个 REST 端点（GET /api/catalog/bundles, GET /api/logs/recent-routing-rules）需端到端冒烟；home 页重做为跨模块联调场景。 |
