...
|---|---|---|---|
| scr | **true** | explicit | 跨 transports + plugins + ui 三模块联调：UI 切换 Switch → REST 写入 → bifrost 内部加载 → KeyPoolFilter 绑定。属跨 role 协作 + 跨模块联调，启用 scenario track 验证 KeyPoolFilter 静默失效排查 |
