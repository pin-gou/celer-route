#!/usr/bin/env bash
# local environment describe_env hook: 描述 prepare_env 执行后的预期环境基线状态
  ...
  local:
    ...
    config_resources:
      ...
      - name: fixture-source-dir
        ...
        scope: environment
        description: "种子数据 JSON 文件源目录，共 12 个 fixture 文件"
        lifecycle: persistent
        tags:
          - fixture
          - seed
    ...
    relations:
      ...
      - from: fixture-source-dir
        ...
        type: references
        criticality: required
        description: "seed.py 读取 fixature 目录的 JSON 文件写入 config-db"
...
pg_exit --status=pass --duration=0 \
  --metadata="env=\"${ENV_NAME}\" caller=\"${CALLER}\""