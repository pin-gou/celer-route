#!/usr/bin/env bash
# ui-dev role tail action: 持续跟随日志 stream（tail -f），Ctrl-C 结束.
# 对应 project.yaml: environments.local.roles.ui-dev.actions.tail
...
echo "=== ui-dev (${PG_INSTANCE_NAME:-ui-dev-1}) 持续跟随日志 (Ctrl-C 结束): $logfile ==="
