...
grep -rn "<feature>" transports/celer-route-http/handlers/ --include='*.go' -l
...
git log --oneline -20 -- 'ui/app/workspace/<feature>/' 'transports/celer-route-http/handlers/<handler>.go'
...
API handlers live in `transports/celer-route-http/handlers/`.

```bash
# Find the relevant handler file
grep -rn '<feature>\|<Feature>' transports/celer-route-http/handlers/ --include='*.go' -l

# Read route registrations
grep -n '/api/' transports/celer-route-http/handlers/<handler>.go

# Read request/response types
grep -n 'type.*Request\|type.*Response' transports/celer-route-http/handlers/<handler>.go

# Read the handler functions for create/update operations
grep -n 'func.*create\|func.*update\|func.*delete\|func.*get' transports/celer-route-http/handlers/<handler>.go
```
...
```bash
grep -A 30 'type Create.*Request\|type Update.*Request' transports/celer-route-http/handlers/<handler>.go
```
