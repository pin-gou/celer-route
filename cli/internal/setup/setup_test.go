...
func TestRenderClaudeCode(t *testing.T) {
	...
	if !strings.Contains(strings.Join(out.Env, "\n"), "export ANTHROPIC_BASE_URL=http://localhost:8080/anthropic") {
...
func TestRenderCodex(t *testing.T) {
	...
	if len(out.Env) != 1 || !strings.Contains(out.Env[0], "CELER_ROUTE_API_KEY=sk-bf-abc") {
...
func TestDispatch(t *testing.T) {
	cases := []Agent{Opencode, ClaudeCode, Codex, OpenAICompatible, Cursor}
	for _, a := range cases {
